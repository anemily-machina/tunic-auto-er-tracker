...
function GetClicked(position) {
    ...
    if (position.x >= 0 &&
        ...
        position.y >= game.layer_height - icons.settings.naturalHeight * 4 &&
        position.y <  game.layer_height)
    {
        if (position.x <= icons.settings.naturalWidth * 4) {
            return { type: TYPE_CONFIG, target: "settings" };
        }
        else if (position.x <= icons.settings.naturalWidth * 4 + CONFIG_XOFFSET + icons.help.naturalWidth) {
