...
const CHECKS_HEIGHT = 225;
const CHECKS_WIDTH  = 375;
const CHECKS_COLOR    = "#f7e1b4";
const CHECKS_FONTSIZE = "102px";
...
const WARP_LINE_YOFFSET = 30;
const WARP_FONT_SIZE    = 30;
...
function InitRendering() {
    ...
    for (let i = 0; i < LAYER_COUNT; ++i) {
        ...
        c.context.imageSmoothingEnabled = true;
        c.rerender = true;
        c.skip = false;
        layers.push(c);
    }
...
function RenderSettings(context) {
    ...
    let v = {
        x: 0,
        y: layer.canvas.height - icons.settings.naturalHeight * 4,
        w: icons.settings.naturalWidth * 4,
        h: icons.settings.naturalHeight * 4
    };
...
function RenderRemainingIcon(context) {
    ...
    let v = {
        ...
        y: layer.canvas.height - icons.settings.naturalHeight*4 - CHECKS_YOFFSET/2,
        w: CONFIG_HEIGHT,
        h: CONFIG_HEIGHT,
    };
    ...
    if (current_state == STATE_LINK1 || current_state == STATE_ITEMLINK) {
        ...
        if (warp.link_type == LINKTYPE_WARP && warp.text_offset) {
            if (warp.text_offset.x) {
                offset.x = game.frame.naturalWidth * warp.text_offset.x;
            }
            if (warp.text_offset.y) {
                offset.y = game.frame.naturalHeight * warp.text_offset.y; }
        }
        return {
...
function RenderRemainingText(context) {
    ...
    context.save(); {
        ...
        let text_position = {
            x: game.left_width - CHECKS_XOFFSET + MARK_SIZE + CONFIG_HEIGHT - 250,
            y: layer.canvas.height - CHECKS_YOFFSET - 60,
        };
...
function GetWarpRenderInfo(warp) {
    ...
    if (warp.link_type) {
        if (warp.link_type == LINKTYPE_MARK) {
            ...
            info.type = "text";
            // Draw location/warp
            // warp frame position and size
            info.x = rendered_location.x + warp.x * rendered_location.scale - game.frame.naturalWidth/8 + 20;
            info.y = rendered_location.y + warp.y * rendered_location.scale - game.frame.naturalHeight/16;
            // frame height and width, frame size
            info.w = game.frame.naturalWidth/4;
            info.h = game.frame.naturalHeight/4;
            info.image = game.frame;

            info.text_position = {
                x: rendered_location.x + warp.x*rendered_location.scale + 22,
    ...
    // entrance question mark icon size
    if (info.type == "image") {
        ...
        info.w = info.image.naturalWidth*2;
        info.h = info.image.naturalHeight*2;
